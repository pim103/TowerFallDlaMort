...
{
    public class MCTSAgentOld : Agent
    {
        ...
        private ref MCTSTreeNode GetMaxChoosingValue(ref MCTSTreeNode root, ref MCTSTreeNode newMaxNode)
        {
            if(root.nodeChoosingValue > -1001){
                ...
                if (root.childNodes!=null)
                {
                    ...
                    while (childrenToCheck<root.childNodes.Count)
                    {
                        Debug.Log(childrenToCheck);
                        Debug.Log(root.childNodes.Count);
                        Debug.Log(root.depth);
                        Debug.Log(root.childNodes[childrenToCheck].actions[0]+" "+root.childNodes[childrenToCheck].actions[1]);
        ...
        private ref MCTSTreeNode GetMaxChoosingValueNotRecursive(ref MCTSTreeNode root)
        {
            ref MCTSTreeNode newMaxNode = ref root;
            MCTSTreeNode currentNode = root;
            while (currentNode.childNodes!=null)
            {
